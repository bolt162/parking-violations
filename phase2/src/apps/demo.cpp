...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: " << argv[0]
                  << " <csv_file> [--indexed] [--parallel] [--threads=N]"
                  << std::endl;
        std::cerr << "  --indexed   Use IndexedSearch (sorted indices + binary search)"
                  << std::endl;
        std::cerr << "  --parallel  Use ParallelSearch (OpenMP parallelized scan)"
                  << std::endl;
        std::cerr << "  --threads=N Set number of OpenMP threads" << std::endl;
    ...
    const char* engine_label = use_indexed ? "IndexedSearch"
