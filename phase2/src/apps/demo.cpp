...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: " << argv[0]
                  << " <csv_file> [--indexed] [--threads=N]"
                  << std::endl;
        std::cerr << "  --indexed   Use IndexedSearch (sorted indices + binary search)"
                  << std::endl;
        std::cerr << "  --threads=N Set number of OpenMP threads (default: 6)"
                  << std::endl;
        std::cerr << "  default     Use ParallelSearch (OpenMP parallelized scan)"
                  << std::endl;
        return 1;
    }
